...
const GIRL_ONLY_CATS = ['skirt', 'stocking', 'onepiece', 'bow'];
...
function updateBase(num) {
  ...
  if (!img) return;
  if (bellyActive[num] || genitalActive[num]) {
    img.onerror = () => { img.onerror = null; img.src = `game-images/base${num}.png`; };
    img.src = `game-images/base${num}_1.png`;
  } else {
    img.onerror = null;
    img.src = `game-images/base${num}.png`;
  }
}
...
const CONFLICTS = {
  ...
  onepiece2: ['topunderwear2','bottomunderwear2'],
  topunderwear2: ['onepiece2'], bottomunderwear2: ['onepiece2'],
  dress2: ['top2','pants2','skirt2','sweatshirt2','bunnysuitbow2'],
  bunnysuitbow2: ['dress2','jacket2'],
  jacket2: ['bunnysuitbow2'],
  stocking2: ['socks2'], socks2: ['stocking2'],
...
function toggleVisibility(itemId, categoryName) {
  ...
  if (making_visible) {
    ...
    if (/^(skirt|dress)\d/.test(categoryName)) {
      setWindState(windOn);
    }
  }
}
...
async function setItemNamedColor(itemId, colorName) {
  ...
  // Sync wind variant
  const m = /^((?:skirt|dress)\d+_\d+)(w?)\.png$/.exec(String(itemId));
  if (m) {
...
function setWindState(on) {
  windOn = on;
  const charNums = Object.keys(CHARACTER_GENDERS).map(Number);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.char-wrap').forEach(wrap => {
    ...
    if (gender === 'girl') {
      _addOverlayImg(wrap, `genital-overlay${n}`, `game-images/base${n}_genital.png`);
